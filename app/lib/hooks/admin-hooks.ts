...
export function useGetAdminCampaignById(id?: string) {
    const session = useSession();

    const { data, isPending, isSuccess, isError, error, refetch } = useQuery<AdminCampaignResponse, AxiosError>({
        queryKey: ['admin-campaigns'],
        queryFn: () => getAdminCampaignById(session.data?.accessToken ?? '', id),
        enabled: !!id
    });
...
export function useGetAdminNotifications() {
    ...
    const { data, isPending, isSuccess, isError, error } = useQuery<AdminNotificationsResponse[], AxiosError>({
        queryKey: ['admin-notifications'],
        queryFn: () => getAdminNotifications(session.data?.accessToken ?? ''),
        enabled: session.status === 'authenticated'
    });
...
export function useAdminDeleteNotification({ onSuccess, onError }: { onSuccess?: () => void, onError?: (error: AxiosError) => void }) {
    const session = useSession();

    const { mutate, data, isPending, isSuccess, isError, error } = useMutation<any, AxiosError, string>({
        mutationFn: (id: string) => adminDeleteNotification(session.data?.accessToken ?? '', id),
        onSuccess(data, variables, context) {
            onSuccess?.();
        },
        onError(error, variables, context) {
